...
import { refreshWarehouseStatus } from "../warehouses/helpers/warehouses.helpers";
...
export class InventoryService {
  ...
  async createInventory(dto: CreateInventoryDto, userId: string) {
    // Validate references (keep these outside transaction for early failure)
    ...
    const [rack, bin] = await Promise.all([
      this.prisma.rack.findUnique({ where: { id: dto.rackId } }),
    ...
    // If exists: update inventory row and increment bin inside a transaction (and refresh status)
      ...
      const result = await this.prisma.$transaction(async (tx) => {
        const updatedInventory = await tx.inventory.update({
          ...
          data: {
            ...
            specialHandling:
              dto.specialHandling ?? existingInventory.specialHandling,
            updatedAt: new Date(),
          },
        });
        ...
        await tx.bin.update({
          where: { id: dto.binId },
          data: { currentQty: { increment: 1 } },
        });

        // refresh warehouse status using the transaction client for atomicity
        const warehouseId = dto.warehouseId;
        await refreshWarehouseStatus(tx as any, warehouseId);

        return { updatedInventory, warehouseId };
      });

      // do not await email send here (no change for updates) — return updatedInventory
      return result.updatedInventory;
    }

    // create new inventory + shipment + status history + increment bin all in a single transaction
    const txResult = await this.prisma.$transaction(async (tx) => {
      ...
      const createdShipment = await tx.shipment.create({
        data: {
          ...
          origin: this.safeParseLocation(dto.origin),
          destination: this.safeParseLocation(dto.destination),
      ...
      // refresh warehouse status within transaction
      await refreshWarehouseStatus(tx as any, dto.warehouseId);

      return { newInventory, createdShipment };
    });

    // after commit: send email (outside tx). If email fails we already committed DB.
    const createdShipment = txResult.createdShipment;
    try {
      if (user.email) {
        const originObj = this.safeParseLocation(
          createdShipment.origin ?? dto.origin
        );
        const destinationObj = this.safeParseLocation(
          createdShipment.destination ?? dto.destination
        );
        ...
        await this.mailer.sendShipmentCreated(user.email, {
          ...
          estimatedDelivery: this.prettyDate(
            createdShipment.deliveryDate ?? dto.deliveryDate
          ),
          status: "In warehouse",
  ...
  async createInventoryLocation(
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const warehouseId = bin.rack.zone.warehouseId;
      await refreshWarehouseStatus(tx as any, warehouseId);
   ...
   */
  private validateSpecialHandling(
    bin: any,
    tempMin: number | null,
    tempMax: number | null,
    isHazardous: boolean
    ...
    if (tempMin !== null || tempMax !== null) {
      if (bin.tempMin === null || bin.tempMax === null) {
        throw new BadRequestException(
          "This bin does not support temperature-controlled storage"
        );
      }
      if (tempMin !== null && tempMin < bin.tempMin) {
        throw new BadRequestException(
          `Bin minimum temperature (${bin.tempMin}°C) cannot accommodate required minimum (${tempMin}°C)`
        );
      }
      if (tempMax !== null && tempMax > bin.tempMax) {
        throw new BadRequestException(
          `Bin maximum temperature (${bin.tempMax}°C) cannot accommodate required maximum (${tempMax}°C)`
        );
      }
    }
    ...
    if (isHazardous && !bin.allowsHazardous) {
      throw new BadRequestException(
        "This bin does not allow hazardous materials storage"
      );
    }
  }
  ...
  async moveOrUpdateInventoryLocation(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (targetBinId !== loc.binId) {
        ...
        const sourceWarehouseId = sourceBin.rack.zone.warehouseId;
        const targetWarehouseId = targetBin.rack.zone.warehouseId;
        await refreshWarehouseStatus(tx as any, sourceWarehouseId);
        if (targetWarehouseId !== sourceWarehouseId) {
          await refreshWarehouseStatus(tx as any, targetWarehouseId);
        }
        ...
        const warehouseId = sourceBin.rack.zone.warehouseId;
        await refreshWarehouseStatus(tx as any, warehouseId);
  ...
  async deleteInventoryLocation(id: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      // load bin (inside tx) so we have up-to-date currentQty and warehouse id
      ...
      // optional safety check: prevent negative counts
      if (loc.qty > bin.currentQty) {
        throw new BadRequestException(
          `Cannot remove ${loc.qty} items from bin (currentQty: ${bin.currentQty}).`
        );
      }
      ...
      // refresh warehouse status atomically (use tx client)
      const warehouseId = bin.rack.zone.warehouseId;
      await refreshWarehouseStatus(tx as any, warehouseId);
  ...
  async listInventoryLocations(filter?: {
    ...
    if (filter?.lotNumber)
      where.lotNumber = { contains: filter.lotNumber, mode: "insensitive" };
    if (filter?.isHazardous !== undefined)
      where.isHazardous = filter.isHazardous;
  ...
  async getInventoryByWarehouseFormatted(
    warehouseId: string,
    page = 1,
    limit = 10
  ) {
    ...
    const formatted = await Promise.all(
      inventories.map(async (inv) => {
        ...
        if (inv.specialHandling) {
          try {
            specialHandlingParsed =
              typeof inv.specialHandling === "string" &&
              inv.specialHandling.trim().startsWith("{")
                ? JSON.parse(inv.specialHandling)
                : inv.specialHandling;
          } catch {
            specialHandlingParsed = inv.specialHandling;
          }
