...
export class InventoryService {
   ...
   async deleteInventoryLocation(id: string) {
      ...
      return this.prisma.$transaction(async (tx) => {
         ...
         if (loc.qty > bin.currentQty) {
            throw new BadRequestException(`Cannot remove ${loc.qty} items from bin (currentQty: ${bin.currentQty}).`);
         }
   ...
   async getInventoryByWarehouseFormatted(warehouseId: string, page = 1, limit = 10) {
      ...
      // Batch fetch shipments and build a lookup map by orderId
      const shipmentIds = inventories.map((inv) => inv.shipmentId).filter(Boolean);
      const shipments = await this.prisma.shipment.findMany({
         where: { orderId: { in: shipmentIds } },
         select: { orderId: true, clientName: true, pickupDate: true, deliveryDate: true },
      });
      const shipmentMap = new Map(shipments.map((s) => [s.orderId, s]));

      // Batch fetch racks and bins to avoid N+1 queries
      const rackIds = inventories.map((inv) => inv.rackId).filter(Boolean) as string[];
      const binIds = inventories.map((inv) => inv.binId).filter(Boolean) as string[];
      ...
      const formatted = inventories.map((inv) => {
         ...
         const shipment = shipmentMap.get(inv.shipmentId);

         return {
            inventoryId: inv.id,
            clientName: inv.clientName || shipment?.clientName || 'N/A',
            shipmentId: inv.shipmentId || 'N/A',
            rack: rackMap.get(inv.rackId ?? '')?.name || 'N/A',
            bin: binMap.get(inv.binId ?? '')?.name || 'N/A',
            status: inv.status || 'N/A',
            condition: inv.condition || 'N/A',
            specialHandling: specialHandlingParsed,
            arrivalDate: shipment?.pickupDate?.toISOString().split('T')[0] || 'N/A',
         };
      });
