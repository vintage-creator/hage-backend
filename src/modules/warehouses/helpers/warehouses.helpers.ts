 ...
 */
export async function getWarehouseUsage(prisma: PrismaClient | Prisma.TransactionClient, warehouseId: string) {
 ...
 */
export async function refreshWarehouseStatus(prisma: PrismaClient | Prisma.TransactionClient, warehouseId: string) {
   ...
   const { used, reserved, usedPlusReserved } = await getWarehouseUsage(prisma, warehouseId);

   const newStatus = wh.totalCapacity > 0 && usedPlusReserved >= wh.totalCapacity ? WarehouseStatus.INACTIVE : WarehouseStatus.ACTIVE;
