...
import { ApiBearerAuth, ApiOperation, ApiResponse, ApiTags, ApiParam, ApiBody, ApiQuery, ApiConsumes } from '@nestjs/swagger';
...
export class WarehousesController {
   ...
   async create(@Body() dto: CreateWarehouseDto, @Request() req: any): Promise<any> {
   ...
   async list(@Query('companyId') companyId?: string, @Query('page', new DefaultValuePipe(1), ParseIntPipe) page = 1, @Query('perPage', new DefaultValuePipe(20), ParseIntPipe) perPage = 20) {
   ...
   @UsePipes(new ValidationPipe({ whitelist: true, transform: true }))
   async createZone(@Param('id') id: string, @Body() dto: CreateZoneDto): Promise<any> {
   ...
   @UsePipes(new ValidationPipe({ whitelist: true, transform: true }))
   async createRack(@Param('zoneId') zoneId: string, @Body() dto: CreateRackDto): Promise<any> {
   ...
   @UsePipes(new ValidationPipe({ whitelist: true, transform: true }))
   async createBin(@Param('rackId') rackId: string, @Body() dto: CreateBinDto): Promise<any> {
   ...
   @UsePipes(new ValidationPipe({ whitelist: true, transform: true }))
   async update(@Param('id') id: string, @Body() dto: UpdateWarehouseDto): Promise<any> {
   ...
   async remove(@Param('id') id: string, @Query('force') force?: boolean): Promise<any> {
