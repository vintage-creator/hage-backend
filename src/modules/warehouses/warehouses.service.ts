...
export class WarehousesService {
   ...
   */
  private async generateWarehouseStructure(
    warehouseId: string,
    warehouse: any,
    numZones: number,
    numRows: number,
    ...
    for (let i = 0; i < numZones; i++) {
      const zoneName =
        standardZoneTypes[i] || `Zone ${String.fromCharCode(65 + i)}`;
    ...
    for (const zone of zones) {
      ...
      for (let rowIndex = 1; rowIndex <= numRows; rowIndex++) {
        for (
          let r = 0;
          r < racksPerRow && rackCounterForZone <= racksPerZone;
          r++
        ) {
          ...
          for (let b = 1; b <= numBinsPerRack; b++) {
            ...
            const locationCode = `${zoneLetter}-R${rowIndex}-${rackName.replace(
              "Rack ",
              "R"
            )}-B${String(b).padStart(3, "0")}`; // e.g. A-R1-RA1-B001
  ...
  async createWarehouse(dto: CreateWarehouseDto, authUserId?: string) {
    ...
    if (
      ...
      dto.numZones > 0 &&
      dto.numRacks > 0 &&
      dto.numBinsPerRack > 0
    ) {
      try {
        structureGenerated = await this.generateWarehouseStructure(
          warehouse.id,
          warehouse,
          dto.numZones,
          dto.numRows!,
    ...
    return {
      ok: true,
      message: structureGenerated
        ? "Warehouse created successfully with structure generated"
  ...
  async suggestLocation(
    ...
    const suitableBins = availableBins.filter((bin) => {
      ...
      if (
        ...
        availableCapacity < options.requiredCapacity
      )
        return false;
    ...
    for (const bin of suitableBins) {
      ...
      rackEntry.bins.push({
        ...
        temperatureRange: bin.tempMin
          ? `${bin.tempMin}°C - ${bin.tempMax}°C`
          : null,
      });
    }

    // Pick one best bin (e.g., most available capacity)
    const allBins = categorized.flatMap((z) =>
      z.racks.flatMap((r: any) =>
        r.bins.map((b: any) => ({ ...b, rackId: r.id, zoneId: z.id }))
      )
    );

    const bestBin = allBins.sort(
  ...
  async getRacksForWarehouse(
    ...
    const items = racks.map((rack) => {
      const totalStock = rack.bins.reduce(
        (sum, b) => sum + (b.currentQty || 0),
        0
      );
