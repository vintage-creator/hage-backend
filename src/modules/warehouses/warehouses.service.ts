...
import { refreshWarehouseStatus, getWarehouseUsage } from './helpers/warehouses.helpers';
...
export class WarehousesService {
   ...
   async listWarehouses(companyId?: string, page = 1, perPage = 20) {
      ...
      const items = await Promise.all(
         warehouses.map(async (w) => {
            ...
            const computedStatus = usedPlusReserved >= (w.totalCapacity ?? 0) ? 'INACTIVE' : w.status;
   ...
   async updateWarehouse(id: string, dto: UpdateWarehouseDto) {
      ...
      const computedStatus = totalCap > 0 && usedPlusReserved >= totalCap ? (WarehouseStatus.INACTIVE as WarehouseStatus) : (wh.status as WarehouseStatus);
      ...
      if (dto.status === WarehouseStatus.ACTIVE && computedStatus === WarehouseStatus.INACTIVE) {
         throw new BadRequestException(`Cannot set status to ACTIVE because the warehouse is full (available: ${available}).`);
      }
