...
export class ShipmentsService {
    ...
    try {
      ...
      for (const field of requiredFields) {
        if (
          ...
          (typeof dto[field] === "object" &&
            Object.keys(dto[field]).length === 0)
        ) {
        ...
        if (uploadedDocs.length > 0) {
          await tx.shipmentDocument.createMany({
            data: uploadedDocs.map((doc, index) => ({
              shipmentId: createdShipment.id,
              docType: this.detectDocumentType(
      ...
      if (dto.email) {
        await this.mailer.sendShipmentCreated(dto.email, {
          ...
          trackingUrl: `${this.cfg.get("APP_URL")}/shipments/track/${
            shipment.orderId
          }`,
        });
      }
  ...
  async acceptAndAssign(
    ...
    if (
      user.kind !== "LOGISTIC_SERVICE_PROVIDER" &&
      user.role !== "CROSS_BORDER_LOGISTICS"
    ) {
    ...
    if (updated.email) {
      const originObj = this.safeParseLocation(
        updated.origin ?? shipment.origin
      );
      const destinationObj = this.safeParseLocation(
    ...
    if (dto.status === ShipmentStatus.ACCEPTED) {
      ...
      if (shipment.assignedTransporterId !== updatedBy)
        throw new ForbiddenException(
          "Only assigned transporter can update to picked up"
        );
      ...
      if (
        ...
        throw new ForbiddenException(
          "Only LSP and order creator can cancel orders"
        );
      }
    } else if (
      [ShipmentStatus.EN_ROUTE_TO_PICKUP, ShipmentStatus.IN_TRANSIT].includes(
        dto.status
      )
    ) {
      if (
        user.kind !== "LOGISTIC_SERVICE_PROVIDER" &&
        shipment.assignedTransporterId !== updatedBy
      ) {
      ...
      if (
        user.kind !== "LOGISTIC_SERVICE_PROVIDER" &&
        user.role !== "LAST_MILE_PROVIDER"
      ) {
        throw new ForbiddenException(
          "Only LSP or last mile provider can complete orders"
        );
      }
    }
  ...
  async findAllForTransporter(
    transporterId: string,
    filters: FilterShipmentDto
  ) {
    ...
    if (
      !user ||
      (user.role !== "TRANSPORTER" && user.role !== "LAST_MILE_PROVIDER")
    ) {
  ...
  async findAllForCustomer(customerId: string, filters: FilterShipmentDto) {
    ...
    if (
      !user ||
      !["ENTERPRISE", "DISTRIBUTOR", "END_USER"].includes(user.kind)
    ) {
  ...
  async getDashboardAnalytics(userId: string): Promise<AnalyticsResponseDto> {
    ...
    const completedShipmentsWithDates = shipments.filter(
      (s) => s.status === "COMPLETED" && s.pickupDate && s.deliveryDate
    );

    let averageDeliveryTimeMinutes = 0;
    if (completedShipmentsWithDates.length > 0) {
      const totalDeliveryTime = completedShipmentsWithDates.reduce(
        (sum, shipment) => {
          // Find actual pickup time from status history
          const pickupHistory = shipment.statusHistory.find(
            (h) => h.status === "PICKED_UP"
          );
          const completedHistory = shipment.statusHistory.find(
      ...
      averageDeliveryTimeMinutes = Math.round(
        totalDeliveryTime / completedShipmentsWithDates.length
      );
    }
    ...
    // Status breakdown
    const byStatus = {
      pending: shipments.filter((s) => s.status === "PENDING_ACCEPTANCE")
        .length,
  ...
  private buildWhereClauseForUser(user: {
    id: string;
    kind: string;
    role: string | null;
  }): any {
  ...
  private async getShipmentAnalytics(where: any) {
    const [totalShipments, pendingAcceptance, inTransit, completed, cancelled] =
      await Promise.all([
        this.prisma.shipment.count({ where }),
  ...
  private validateStatusTransition(
    ...
    const validTransitions: Record<string, ShipmentStatus[]> = {
      ...
      [ShipmentStatus.ACCEPTED]: [
        ShipmentStatus.EN_ROUTE_TO_PICKUP,
        ShipmentStatus.CANCELLED,
      ],
      [ShipmentStatus.EN_ROUTE_TO_PICKUP]: [
        ShipmentStatus.PICKED_UP,
