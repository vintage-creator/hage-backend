...
export class ShipmentsService {
   ...
   async findAll(filters: FilterShipmentDto, userId: string) {
      ...
      } else if (user.kind === 'ENTERPRISE' || user.kind === 'DISTRIBUTOR' || user.kind === 'INDIVIDUAL') {
   ...
   async findAllForCustomer(customerId: string, filters: FilterShipmentDto) {
      ...
      if (!user || !['ENTERPRISE', 'DISTRIBUTOR', 'INDIVIDUAL'].includes(user.kind)) {
   ...
   async getDashboardAnalytics(userId: string): Promise<AnalyticsResponseDto> {
      ...
      const completedCount = shipments.filter((s: any) => !['PENDING', 'IN_WAREHOUSE'].includes(s.status)).length;
   ...
   async canUserAccessShipment(shipmentId: string, userId: string): Promise<boolean> {
      ...
      if (user.kind === 'ENTERPRISE' || user.kind === 'DISTRIBUTOR' || user.kind === 'INDIVIDUAL') {
