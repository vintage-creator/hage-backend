...
export class ShipmentsController {
  ...
  @ApiOperation({
    ...
    description:
      "Allows authenticated users (LSPs, enterprises, or distributors) to create a new shipment record. Supports uploading shipment-related documents (bill of lading, invoice, etc.).",
  })
  @ApiBody({
    description:
      "Create a new shipment (form-data). Use field name `documents` for file uploads.",
    schema: {
      type: "object",
      properties: {
        orderId: { type: "string" },
  ...
  @ApiOperation({
    ...
    description:
      "Generates a unique tracking number for use when creating shipments.",
  })
  generateTracking() {
  ...
  @ApiBody({
    description:
      "Assign a transporter and/or warehouse to a shipment. Only LSP/CROSS_BORDER_LOGISTICS may call.",
  ...
  @ApiOperation({
    ...
    description:
      "Allows Logistic Service Providers (LSPs) to accept and assign a shipment to a transporter or route for fulfillment.",
  })
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      "Filter shipments by status (e.g., PENDING_ACCEPTANCE, ACCEPTED, EN_ROUTE_TO_PICKUP, PICKED_UP)",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Fetches all shipments currently assigned to the authenticated transporter account for fulfillment.",
  })
  async getMyAssignedShipments(
    @Query() filters: FilterShipmentDto,
    @Req() req: any
  ) {
  ...
  @ApiOperation({
    ...
    description:
      "Returns a list of shipments that were created by the currently logged-in customer or enterprise account.",
  })
  async getMyCreatedShipments(
    @Query() filters: FilterShipmentDto,
    @Req() req: any
  ) {
  ...
  @ApiOperation({
    ...
    description:
      "Allows a Logistic Service Provider (LSP) to view all shipments across their managed network. Supports optional filters.",
  })
  async getAllShipmentsAdmin(
    @Query() filters: FilterShipmentDto,
    @Req() req: any
  ) {
  ...
  @ApiOperation({
    ...
    description:
      "Returns key shipment performance metrics — including total shipments, delayed shipments, delivery times, and recent activity.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Fetches detailed shipment information, including cargo details, status, origin, and destination.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Allows authenticated users to track a shipment using its tracking number (orderId).",
  })
  ...
  @ApiBody({
    description:
      "Update shipment details. Fields are mostly optional — include only fields you want to change.",
  ...
  @ApiOperation({
    ...
    description:
      "Allows authorized roles (LSPs, transporters, last-mile providers) to update shipment details such as delivery date or status.",
  })
  update(
    @Param("id") id: string,
  ...
  @ApiOperation({
    ...
    description:
      "Allows the creator or an admin to permanently delete a shipment record. Use with caution — this cannot be undone.",
  })
  ...
  @ApiBody({
    description:
      "Change shipment status. `status` must be one of your ShipmentStatus enum values. `note` is optional.",
  ...
  @ApiOperation({
    ...
    description:
      "Updates the current status of a shipment (e.g., from PENDING to IN_TRANSIT, or DELIVERED). Automatically records status history.",
  })
  async updateStatus(
    @Param("id") shipmentId: string,
