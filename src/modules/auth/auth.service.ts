...
export class AuthService {
	...
	async refreshToken(refreshToken: string) {
		...
		await this.prisma.$transaction([
			this.prisma.refreshToken.delete({ where: { id: existingToken.id } }),
	...
	async requestPasswordReset(email: string) {
		const genericResp = {
			ok: true,
			message: "Password Reset Email Sent. Your password reset email has been successfully sent. It typically arrives within 5 minutes but may take up to 24 hours. If you have not received it after 24 hours, please contact our support team for assistance.",
		};
